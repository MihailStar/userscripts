// ==UserScript==
// @name search.imgsrc
// @description Добавляет функциональность сортировки альбомов в поисковой выдаче на imgsrc.ru
// @version 0.0.1
// @license WTFPL
// @author MihailStar
// @include /^https?:\/\/imgsrc\.ru\/main\/search\.php\?.+$/
// @include /^https?:\/\/imgsrc\.ru\/cat\/.+$/
// @grant none
...

{
  class MyEnum {
    ...
    constructor(items) {
      ...
      Object.keys(items).forEach((key) => {
        enumeration[(enumeration[key] = items[key])] = key;
      });
   ...
   */
  const MyView = (() => {
    const firstRows = Symbol('firstRows');
    const lastRows = Symbol('lastRows');
    ...
    return class MyView extends MyEventEmitter {
       ...
       * @param {Array<HTMLTableRowElement>} footerRows
       * @param {HTMLTableElement} container
       */
      constructor(headerRows, footerRows, container) {
        super();

        this[firstRows] = headerRows;
        this[lastRows] = footerRows;
        ...
        const initializeСontrols = () => {
          ...
          [...this[firstRows][0].children].slice(0, -1).forEach((th, index) => {
            th.innerHTML = `
              ${th.innerHTML}
              <br>
              <button
  ...
  const footerRows = [rows[rows.length - 2], rows[rows.length - 1]];
  const informationRows = Reflect.apply(Array.prototype.slice, rows, [1, -2]);

  const myStorage = Reflect.construct(MyStorage, ['album-sort-type']);
  const myModel = Reflect.construct(MyModel, [informationRows]);
  const myView = Reflect.construct(MyView, [headerRows, footerRows, table]);
